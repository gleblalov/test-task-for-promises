...
async function startRecursive() {
    const promises = users.splice(0, 10).map(user => processUser(user.id));
    const total = await Promise.all(promises).then(res => res.reduce((sum, current) => sum + current));
    
    if (users.length === 0) {
      return total;
    } else {
      return total + await startRecursive();
    }
  }
